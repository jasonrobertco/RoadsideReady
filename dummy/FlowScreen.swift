...
struct FlowScreen: View {
    ...
    // toggle this manually when you’re ready to turn AR on
    private let enableAR = true
    
    private var canRunAR: Bool {
        enableAR && !isXcodePreview && !isSimulatorRuntime && ARWorldTrackingConfiguration.isSupported
    }
    ...
    @State private var cameraDeniedMessage = "Camera access is required for AR."
        ...
        Button {
            AVCaptureDevice.requestAccess(for: .video) { granted in
                DispatchQueue.main.async {
                    if granted {
                        cameraEverOpened = true
                        // optional: show initial hint immediately
                        arSession.setStatus("Ready — tap a surface to place", phase: .ready)
                    } else {
                        cameraDeniedMessage = "Enable Camera in Settings to use AR."
...
private struct ARUnavailableInlineView: View {
    var message: String = "AR camera unavailable.\nPress ▶︎ Run to use AR."
