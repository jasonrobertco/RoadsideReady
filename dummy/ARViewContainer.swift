...
struct ARViewContainer: UIViewRepresentable {
    ...
    func makeUIView(context: Context) -> ARView {
        guard ARWorldTrackingConfiguration.isSupported else {
            ...
            // Optional minimal fallback message so users know why AR isn't showing
            let label = UILabel()
            ...
            NSLayoutConstraint.activate([
                ...
                label.centerYAnchor.constraint(equalTo: arView.centerYAnchor),
                label.leadingAnchor.constraint(greaterThanOrEqualTo: arView.leadingAnchor, constant: 16),
                label.trailingAnchor.constraint(lessThanOrEqualTo: arView.trailingAnchor, constant: -16)
            ])
            return arView
        }
    ...
    final class Coordinator: NSObject, ARSessionDelegate {
        ...
        // 1) Selector entrypoint must be nonisolated
        @objc nonisolated func handleTap(_ sender: UITapGestureRecognizer) {
            Task { @MainActor in
                self.handleTapMain(sender)
            }
        }
        
        // 2) Move your existing logic here (same body as before)
        @MainActor
        private func handleTapMain(_ sender: UITapGestureRecognizer) {
        ...
        private func lugIndex(for entity: Entity) -> Int? {
            var e: Entity? = entity
            while let cur = e {
                // 1. Check if the name prefix matches
                if cur.name.hasPrefix("rr_lug_") {
                    // 2. Safely drop the prefix and try to convert the remainder to an Int
                    let suffix = cur.name.dropFirst("rr_lug_".count)
        ...
        nonisolated func session(_ session: ARSession,
            ...
            Task { @MainActor in
                self.sessionCameraDidChange(camera)
            }
        }
        
        private func sessionCameraDidChange(_ camera: ARCamera) {
            guard awaitingReset else { return }
            if case .normal = camera.trackingState {
                awaitingReset = false
                sessionModel.setStatus("Ready — tap a surface to place", phase: .ready)
                autoHideStatus(after: 1.5)
            }
        }
